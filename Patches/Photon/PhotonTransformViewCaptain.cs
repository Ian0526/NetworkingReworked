...
namespace NetworkingReworked
{
    ...
    public static class PhotonTransformViewSerializationPatch
    {
        ...
        public static bool Prefix_OnPhotonSerializeView(PhotonTransformView __instance, PhotonStream stream, PhotonMessageInfo info)
        {
            ...
            if (__instance.GetComponentInParent<PhysGrabObject>() == null) return true;

            if (stream.IsWriting && PhotonNetwork.IsMasterClient && __instance.photonView.IsMine)
            {
            ...
            else if (!__instance.photonView.IsMine)
            {
                ...
                receivedVelocityField.SetValue(__instance, receivedVel);
                receivedAngularVelocityField.SetValue(__instance, receivedAngVel);

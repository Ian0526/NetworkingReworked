...
{
    // Helper method to replace IsMasterClientOrSingleplayer()
    ...
    public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
    {
        ...
        {
            if (code[i].opcode == OpCodes.Call && code[i].operand as MethodInfo == isMasterMethod)
            {
                // Replace: call bool SemiFunc::IsMasterClientOrSingleplayer()
                // With: call bool HurtColliderPatch::IsOwnerOrSingleplayer(GameObject)
                code[i] = new CodeInstruction(OpCodes.Ldarg_0); // this (HurtCollider)
